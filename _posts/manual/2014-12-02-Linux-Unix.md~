...
利用系统调用stat()、lstat()以及fstat()，可以获取与文件有关的信息，其中大部分提取自文件的i节点。
...
lstat()与stat()类似，区别在于如果文件属于符号链接，那么所返回的信息针对的是符号链接本身(而非符号链接所指向的文件)，换句话说就是不解引用。

fstat()则会返回某个打开的文件描述符所指代文件的信息。

系统调用stat()和lstat()无需对文件本身拥有任何权限，但是针对pathname的父目录要有执行(搜索)权限。

上述所有的调用都会在缓冲区返回一个由statbuf指向的stat结构
...
<ul>
	<li>设备ID和i节点号：st_dev字段标识文件所驻留的设备。st_ino字段则包含了文件的i节点号。利用以上两者，可在所有文件系统中唯一识别某个文件。dev_t类型记录了设备的主、辅ID。如果是针对设备的i节点，那么在st_rdev字段则包含设备的主、辅ID。</li>
